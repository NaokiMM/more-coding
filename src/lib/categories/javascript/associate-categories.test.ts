/**
 * JavaScript Associate のカテゴリ定義（associate-categories.ts）のユニットテスト
 *
 * 対象: src/lib/categories/javascript/associate-categories.ts
 * カテゴリが1件以上あること、id の一意性、.json ファイル指定、グラデーション指定を検証する
 *
 * 動作方法:
 *   npm test  … 全テストを実行
 *   npm test -- --testPathPattern=javascript/associate-categories  … このファイルのみ実行
 */
import { categoriesData } from "./associate-categories";

describe("categoriesData (JavaScript Associate)", () => {
