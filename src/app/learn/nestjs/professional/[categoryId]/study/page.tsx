/**
 * NestJS Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nestjs/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nestjsProfessionalCategoriesData } from "@/lib/categories/nestjs/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nestjsProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nestjs/professional/${categoryId}/study`;
  const backHref = "/learn/nestjs/professional";
...
export function generateStaticParams() {
  return nestjsProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = nestjsProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nestjs", "professional", category.file);
