/**
 * NestJS Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nestjs/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nestjsExpertCategoriesData } from "@/lib/categories/nestjs/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nestjsExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nestjs/expert/${categoryId}/study`;
  const backHref = "/learn/nestjs/expert";
...
export function generateStaticParams() {
  return nestjsExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = nestjsExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nestjs", "expert", category.file);
