/**
 * NestJS Associate カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nestjs/associate/[categoryId]/study
 *
 * 指定されたカテゴリIDの学習データ（問題集）をS3から取得し、
 * StudyClientに渡します。s3-assets/nestjs/associate/jp/*.json に対応。
 */

import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nestjsAssociateCategoriesData } from "@/lib/categories/nestjs/associate-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nestjsAssociateCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nestjs/associate/${categoryId}/study`;
  const backHref = "/learn/nestjs/associate";
  ...
  if (!locale || !isValidLearnLocale(locale)) {
    return (
      <StudyStartWithLocaleSelect
        ...
        backHref={backHref}
        backLabel="カテゴリ一覧に戻る"
        colorClass="from-slate-700 to-blue-800"
        icon="🔴"
      />
    );
  }
...
export function generateStaticParams() {
  return nestjsAssociateCategoriesData.map((category) => ({
    categoryId: category.id,
  }));
}

async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  const category = nestjsAssociateCategoriesData.find((cat) => cat.id === categoryId);
  if (!category) {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nestjs", "associate", category.file);
