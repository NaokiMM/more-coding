/**
 * Laravel Expert カテゴリ一覧ページ
 *
 * ルート: /learn/laravel/expert
 *
 * このページは、Laravel Expertレベルの学習カテゴリ一覧を表示します。
 * 各カテゴリをクリックすると、そのカテゴリの学習ページに遷移します。
 */
...
export default function LaravelExpertPage() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <nav className="mb-8 flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
          ...
          <span>/</span>
          <Link href="/learn/laravel" className="hover:text-slate-900 dark:hover:text-white transition-colors">
            Laravel
          </Link>
          <span>/</span>
        ...
        <div className="mb-12 text-center">
          <div className="mb-4 inline-flex h-20 w-20 items-center justify-center rounded-xl bg-gradient-to-br from-red-600 to-pink-600 text-white text-3xl font-bold shadow-lg">
            🚀
          </div>
          ...
          <p className="mx-auto max-w-2xl text-lg text-slate-600 dark:text-slate-400">
            高度なLaravelパターンとアーキテクチャを学びます。大規模アプリケーション開発のエキスパートを目指しましょう。
          </p>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {categories.map((category) => (
            <Link
              key={category.id}
              href={`/learn/laravel/expert/${category.id}/study`}
