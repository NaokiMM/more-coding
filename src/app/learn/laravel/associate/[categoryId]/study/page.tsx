/**
 * Laravel Associate カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/laravel/associate/[categoryId]/study
 *
 * 指定されたカテゴリIDの学習データ（問題集）をS3から取得し、
 * StudyClientに渡します。s3-assets/laravel/associate/jp/*.json に対応。
 */

import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as laravelAssociateCategoriesData } from "@/lib/categories/laravel/associate-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = laravelAssociateCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/laravel/associate/${categoryId}/study`;
  const backHref = "/learn/laravel/associate";
...
export function generateStaticParams() {
  return laravelAssociateCategoriesData.map((category) => ({
    categoryId: category.id,
  }));
}

async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  const category = laravelAssociateCategoriesData.find((cat) => cat.id === categoryId);
  if (!category) {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "laravel", "associate", category.file);
