/**
 * Laravel Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/laravel/professional/[categoryId]/study
 *
 * 指定されたカテゴリIDの学習データ（問題集）をS3から取得し、
 * StudyClientに渡します。s3-assets/laravel/professional/jp/*.json に対応。
 */

import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as laravelProfessionalCategoriesData } from "@/lib/categories/laravel/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = laravelProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/laravel/professional/${categoryId}/study`;
  const backHref = "/learn/laravel/professional";
...
export function generateStaticParams() {
  return laravelProfessionalCategoriesData.map((category) => ({
    categoryId: category.id,
  }));
}

async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  const category = laravelProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  if (!category) {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "laravel", "professional", category.file);
