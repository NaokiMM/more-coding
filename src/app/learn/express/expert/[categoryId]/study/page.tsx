...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = expressExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/express/expert/${categoryId}/study`;
  const backHref = "/learn/express/expert";
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "express", "expert", category.file);
