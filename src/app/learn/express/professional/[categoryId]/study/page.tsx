/**
 * Express.js Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/express/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as expressProfessionalCategoriesData } from "@/lib/categories/express/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = expressProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/express/professional/${categoryId}/study`;
  const backHref = "/learn/express/professional";
...
export function generateStaticParams() {
  return expressProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = expressProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "express", "professional", category.file);
