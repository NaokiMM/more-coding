/**
 * Gin Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/gin/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as ginExpertCategoriesData } from "@/lib/categories/gin/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = ginExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/gin/expert/${categoryId}/study`;
  const backHref = "/learn/gin/expert";
...
export function generateStaticParams() {
  return ginExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = ginExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "gin", "expert", category.file);
