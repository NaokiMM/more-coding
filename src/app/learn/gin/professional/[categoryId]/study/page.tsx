/**
 * Gin Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/gin/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as ginProfessionalCategoriesData } from "@/lib/categories/gin/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = ginProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/gin/professional/${categoryId}/study`;
  const backHref = "/learn/gin/professional";
...
export function generateStaticParams() {
  return ginProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = ginProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "gin", "professional", category.file);
