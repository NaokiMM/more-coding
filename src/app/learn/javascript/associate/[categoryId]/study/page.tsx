...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = jsAssociateCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/javascript/associate/${categoryId}/study`;
  const backHref = "/learn/javascript/associate";
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "javascript", "associate", category.file);
