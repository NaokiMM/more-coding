/**
 * JavaScript Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/javascript/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as jsExpertCategoriesData } from "@/lib/categories/javascript/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = jsExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/javascript/expert/${categoryId}/study`;
  const backHref = "/learn/javascript/expert";
...
export function generateStaticParams() {
  return jsExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = jsExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "javascript", "expert", category.file);
