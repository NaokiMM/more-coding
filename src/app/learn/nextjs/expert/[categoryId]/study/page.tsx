/**
 * Next.js Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nextjs/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nextjsExpertCategoriesData } from "@/lib/categories/nextjs/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nextjsExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nextjs/expert/${categoryId}/study`;
  const backHref = "/learn/nextjs/expert";
...
export function generateStaticParams() {
  return nextjsExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = nextjsExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nextjs", "expert", category.file);
