/**
 * Next.js Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nextjs/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nextjsProfessionalCategoriesData } from "@/lib/categories/nextjs/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nextjsProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nextjs/professional/${categoryId}/study`;
  const backHref = "/learn/nextjs/professional";
...
export function generateStaticParams() {
  return nextjsProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = nextjsProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nextjs", "professional", category.file);
