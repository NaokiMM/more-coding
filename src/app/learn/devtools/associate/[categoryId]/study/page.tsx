...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = devToolsAssociateCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/devtools/associate/${categoryId}/study`;
  const backHref = "/learn/devtools/associate";
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "devtools", "associate", category.file);
