/**
 * TypeScript Expert カテゴリ 学習ページ（サーバーコンポーネント）
 * 
 * ルート: /learn/typescript/expert/[categoryId]/study
 * 
...
import { categoriesData as tsExpertCategoriesData } from "@/lib/categories/typescript/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = tsExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/typescript/expert/${categoryId}/study`;
  const backHref = "/learn/typescript/expert";
...
export function generateStaticParams() {
  return tsExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  ...
  // tsExpertCategoriesDataからcategoryIdに対応するカテゴリを検索
  const category = tsExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "typescript", "expert", category.file);
