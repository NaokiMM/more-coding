/**
 * React Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/react/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as reactExpertCategoriesData } from "@/lib/categories/react/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = reactExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/react/expert/${categoryId}/study`;
  const backHref = "/learn/react/expert";
...
export function generateStaticParams() {
  return reactExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = reactExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "react", "expert", category.file);
