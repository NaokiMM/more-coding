/**
 * SEO Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/seo/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as seoProfessionalCategoriesData } from "@/lib/categories/seo/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = seoProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/seo/professional/${categoryId}/study`;
  const backHref = "/learn/seo/professional";
...
export function generateStaticParams() {
  return seoProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = seoProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "seo", "professional", category.file);
