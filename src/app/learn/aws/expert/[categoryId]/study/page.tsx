/**
 * AWS Expert カテゴリ 学習ページ（サーバーコンポーネント）
 * 
 * ルート: /learn/aws/expert/[categoryId]/study
 * 
...
import { categoriesData as awsExpertCategoriesData } from "@/lib/categories/aws/expert-categories";
...
export function generateStaticParams() {
  return awsExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string): Promise<CategoryData> {
  ...
  // awsExpertCategoriesDataからcategoryIdに対応するカテゴリを検索
  const category = awsExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = `${baseUrl}/questions/aws/expert/${category.file}`;
