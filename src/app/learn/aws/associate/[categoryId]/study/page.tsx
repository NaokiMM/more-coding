/**
 * AWS Associate カテゴリ 学習ページ（サーバーコンポーネント）
 * 
 * ルート: /learn/aws/associate/[categoryId]/study
 * 
...
import { categoriesData as awsAssociateCategoriesData } from "@/lib/categories/aws/associate-categories";
...
export function generateStaticParams() {
  return awsAssociateCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string): Promise<CategoryData> {
  ...
  // awsAssociateCategoriesDataからcategoryIdに対応するカテゴリを検索
  const category = awsAssociateCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = `${baseUrl}/questions/aws/associate/${category.file}`;
