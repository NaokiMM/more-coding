/**
 * AWS Professional カテゴリ 学習ページ（サーバーコンポーネント）
 * 
 * ルート: /learn/aws/pro/[categoryId]/study
 * 
...
import { categoriesData as awsProCategoriesData } from "@/lib/categories/aws/pro-categories";
...
export function generateStaticParams() {
  return awsProCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string): Promise<CategoryData> {
  ...
  // awsProCategoriesDataからcategoryIdに対応するカテゴリを検索
  const category = awsProCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = `${baseUrl}/questions/aws/pro/${category.file}`;
