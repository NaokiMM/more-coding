/**
 * Vue.js Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/vue/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as vueProfessionalCategoriesData } from "@/lib/categories/vue/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = vueProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/vue/professional/${categoryId}/study`;
  const backHref = "/learn/vue/professional";
...
export function generateStaticParams() {
  return vueProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = vueProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "vue", "professional", category.file);
