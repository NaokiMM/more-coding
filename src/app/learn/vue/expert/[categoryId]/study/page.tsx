/**
 * Vue.js Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/vue/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as vueExpertCategoriesData } from "@/lib/categories/vue/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = vueExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/vue/expert/${categoryId}/study`;
  const backHref = "/learn/vue/expert";
...
export function generateStaticParams() {
  return vueExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = vueExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "vue", "expert", category.file);
