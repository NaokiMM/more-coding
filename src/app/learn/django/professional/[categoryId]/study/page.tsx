/**
 * Django Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/django/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as djangoProfessionalCategoriesData } from "@/lib/categories/django/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = djangoProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/django/professional/${categoryId}/study`;
  const backHref = "/learn/django/professional";
...
export function generateStaticParams() {
  return djangoProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = djangoProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "django", "professional", category.file);
