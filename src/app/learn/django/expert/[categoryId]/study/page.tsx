/**
 * Django Expert カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/django/expert/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as djangoExpertCategoriesData } from "@/lib/categories/django/expert-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = djangoExpertCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/django/expert/${categoryId}/study`;
  const backHref = "/learn/django/expert";
...
export function generateStaticParams() {
  return djangoExpertCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = djangoExpertCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "django", "expert", category.file);
