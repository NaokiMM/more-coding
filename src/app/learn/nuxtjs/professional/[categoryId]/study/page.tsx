/**
 * Nuxt.js Professional カテゴリ 学習ページ（サーバーコンポーネント）
 *
 * ルート: /learn/nuxtjs/professional/[categoryId]/study
 */

import { notFound } from "next/navigation";
import StudyClient from "./StudyClient";
import StudyStartWithLocaleSelect from "@/components/StudyStartWithLocaleSelect";
import { categoriesData as nuxtjsProfessionalCategoriesData } from "@/lib/categories/nuxtjs/professional-categories";
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nuxtjsProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nuxtjs/professional/${categoryId}/study`;
  const backHref = "/learn/nuxtjs/professional";
...
export function generateStaticParams() {
  return nuxtjsProfessionalCategoriesData.map((category) => ({
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData | null> {
  const category = nuxtjsProfessionalCategoriesData.find((cat) => cat.id === categoryId);
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nuxtjs", "professional", category.file);
