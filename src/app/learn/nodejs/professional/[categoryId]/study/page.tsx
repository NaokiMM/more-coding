 ...
 * StudyClientに渡します。s3-assets/nodejs/professional/*.json に対応。
 */
...
export default async function StudyPage({ params, searchParams }: PageProps) {
  ...
  const category = nodejsProfessionalCategoriesData.find((c) => c.id === categoryId);
  const studyPath = `/learn/nodejs/professional/${categoryId}/study`;
  const backHref = "/learn/nodejs/professional";
...
async function getCategoryData(categoryId: string, locale: LearnLocale): Promise<CategoryData> {
  ...
  try {
    const jsonUrl = getQuestionsJsonUrl(baseUrl, locale, "nodejs", "professional", category.file);
