  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* 一時的な画像アップロードテストUI */}
        <div className="mb-8 rounded-2xl bg-yellow-50 border-2 border-yellow-300 p-6 dark:bg-yellow-900/20 dark:border-yellow-700">
          ...
          <div className="space-y-4">
            <input
              ...
              className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/20 dark:file:text-blue-300"
              onChange={async (e) => {
                const file = e.target.files?.[0];
                if (!file) return;

                try {
                  console.log("画像選択:", file.name, file.type, file.size);

                  // セッション取得
                  const session = await getSession();
                  if (!session) {
                    console.error("認証エラー: セッションが取得できません");
                    return;
                  }
                  ...
                  const presignResponse = await fetch(
                    "https://h7sqt3sfpj.execute-api.ap-northeast-1.amazonaws.com/profile-image/presign",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  if (!presignResponse.ok) {
                    const errorData = await presignResponse.json().catch(() => ({}));
                    console.error("プリサインドURL取得失敗:", presignResponse.status, errorData);
                    return;
                  }

                  const { uploadUrl } = await presignResponse.json();
                  console.log("プリサインドURL取得成功:", uploadUrl);
                  ...
                  if (!uploadResponse.ok) {
                    console.error("S3アップロード失敗:", uploadResponse.status, uploadResponse.statusText);
                    return;
                  }
